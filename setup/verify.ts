 ...
 *
 * Uses sql.js (no native dependencies), platform-aware service checks.
 */
...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.qwqnanoclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.qwqnanoclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active qwqnanoclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        const output = execSync(`${prefix} list-unit-files`, { encoding: 'utf-8' });
        if (output.includes('qwqnanoclaw')) {
          service = 'stopped';
        }
      } catch {
    ...
    const pidFile = path.join(projectRoot, 'qwqnanoclaw.pid');
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'qwqnanoclaw', 'mount-allowlist.json'))) {
