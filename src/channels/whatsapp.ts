...
export class WhatsAppChannel implements Channel {
  ...
  private async connectInternal(onFirstOpen?: () => void): Promise<void> {
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (qr) {
        const msg =
          'WhatsApp authentication required. Run /setup in Qwen Code.';
        logger.error(msg);
        exec(
          `osascript -e 'display notification "${msg}" with title "QwQnanoclaw" sound name "Basso"'`,
        );
  ...
  async syncGroupMetadata(force = false): Promise<void> {
    ...
    try {
      ...
      setLastGroupSync(new Date().toISOString());
